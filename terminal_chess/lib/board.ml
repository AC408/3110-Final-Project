...
let update_turn (board : model) (change : change) =
  ...
  | Change -> 
    ...
    let blackknight1 = place_piece (Some ('b', 8)) Black Knight "|  ♞   " false
    let blackbishop1 = place_piece (Some ('c', 8)) Black Bishop "|  ♝   " false
    let blackqueen = place_piece (Some ('d', 8)) Black Queen "|  ♛   " false
    let blackking = place_piece (Some ('e', 8)) Black King "|  ♚   " false
    let blackbishop2 = place_piece (Some ('f', 8)) Black Bishop "|  ♝   " false
    let blackknight2 = place_piece (Some ('g', 8)) Black Knight "|  ♞   " false
    ...
    let whiteknight1 = place_piece (Some ('b', 1)) White Knight "|  ♘   " false
    let whitebishop1 = place_piece (Some ('c', 1)) White Bishop "|  ♗   " false
    let whitequeen = place_piece (Some ('d', 1)) White Queen "|  ♕   " false
    let whiteking = place_piece (Some ('e', 1)) White King "|  ♔   " false
    let whitebishop2 = place_piece (Some ('f', 1)) White Bishop "|  ♗   " false
    let whiteknight2 = place_piece (Some ('g', 1)) White Knight "|  ♘   " false
