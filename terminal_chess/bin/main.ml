...
| Some piece ->
  ...
  | "Queen" -> 
    if Piece.get_color piece = White then
      Some (Piece.place_piece (Piece.get_position piece) 
...
and mover_init board =
  let avail_wp = ref [] in let avail_bp = ref [] in let w_k = ref whiteking in 
  ...
  if wrong_input (in_command) = "wrong move" then mover_init board
    ...
    begin
    if List.hd (Command.parse (in_command)) <> "move" || char_in_range 
      (Command.parse_mod (in_command))
    then 
      ...
      if (filled_input board (Array.get (Command.check1 input) 
        ((Char.code input.[1]) - 97)))
      then 
        ...
        let i_p = Array.get i_pr ((Char.code input.[1]) - 97) in (*ip*)
        let o_p = Array.get o_pr ((Char.code input.[3]) - 97) in (*op*)
        if check_piece i_p input o_p = false then (
          ...
          let new_board = (if (o_p <> None && 
            Command.castle i_p input o_p = false) then 
              {board with graveyard = rep o_p ::  board.graveyard} else board) 
            in (* new_board checks if output position is occupied *)
          let moved_piece = 
            match (Array.get i_pr ((Char.code input.[1]) - 97)) with
            | None -> None
            | Some piece -> Some (Piece.place_piece (Some (input.[3], 
            (int_of_char input.[2] - int_of_char '0'))) (Piece.get_color piece) 
            (Piece.get_level piece) (Piece.get_rep piece) true)
          in
          let moved_piece2 =
            match (Array.get o_pr ((Char.code input.[3]) - 97)) with
            | None -> None
            | Some piece -> Some (Piece.place_piece (Piece.get_position piece) 
          ...
          Array.set o_pr ((Char.code input.[3]) - 97) 
          (promote_check input moved_piece);
          (let new_board2 = {new_board with model = 
            update_turn new_board.model Board.Change} in
          ...
          | Some x -> (if x.color = White then avail_wp:=x::
            !avail_wp else avail_bp := x::!avail_bp); 
          if x = whiteking then w_k:= x else if x = blackking then b_k:=x) 
            new_board2.r1;
          Array.iter (fun y -> match y with | None -> () 
          | Some x -> (if x.color = White then avail_wp:=x::
            !avail_wp else avail_bp := x::!avail_bp); 
          ...
          if (board.model.turn = White) then 
            if (Command.checkmated !avail_wp !avail_bp !b_k new_board2.r1 
          ...
          new_board2.r6 new_board2.r7 new_board2.r8)
            ...
            if (Command.checkmated !avail_bp !avail_wp !w_k new_board2.r1 
              new_board2.r2 new_board2.r3 new_board2.r4 new_board2.r5 
          new_board2.r6 new_board2.r7 new_board2.r8)
              then (print_endline "Checkmate!"; false)
